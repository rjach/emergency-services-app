/**
 * RapidAid Dashboard — Google Maps with Live Geolocation
 * Falls back to Kathmandu center if geolocation is unavailable.
 */
...
function initMap() {
  ...
  // Request live location (marker + circle created inside)
  requestGeolocation();
  ...
  // Recenter always goes to the latest known user location
  document.getElementById('recenter').addEventListener('click', function () {
    if (userLocation) {
      map.panTo(userLocation);
      map.setZoom(DEFAULT_ZOOM);
    }
    // Also refresh the position
    requestGeolocation();
  });
}
